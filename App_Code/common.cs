...
public static class Common
{
    ...
    public static void UserSettings(ArrayList alCommonControls)
    {
        try
        {
            ...
            var EmailCookie = HttpContext.Current.Request.Cookies["UserEmail"];
            if (httpCookie == null)
            {
            ...
            else
            {
                ...
                if (httpCookie != null)
                {
                    HttpContext.Current.Session["UserID"] = httpCookie.Value;
                    if (usernameCookie != null) HttpContext.Current.Session["Username"] = usernameCookie.Value;
                    if (EmailCookie != null) HttpContext.Current.Session["UserEmail"] = EmailCookie.Value;
                    string selectQuery =
    ...
    public static void AdminSettings(ArrayList alCommonControls)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    string selectQuery =
                        string.Format("SELECT UserID,U_Firstname + ' ' + U_Lastname as Fullname,U_ProfilePic From Tbl_Users  WHERE Tbl_Users.UserID={0}", IEUtils.ToInt(httpCookie.Value));
                    SqlDataReader dr = db.ExecuteReader(selectQuery);
                    if (dr.HasRows)
                    {
                        dr.Read();
    ...
    public static void GetAdminSummary(List<Label> lstLabels)
    {
        try
        {
            ...
            lstLabels[3].Text = " (" + db.GetExecuteScalar("SELECT COUNT(Tbl_Users.UserID) FROM Tbl_Users INNER JOIN Tbl_Brands ON Tbl_Users.UserID = Tbl_Brands.UserID WHERE (Tbl_Users.U_Type = 'Brand') AND (Tbl_Users.IsApproved IS NULL) AND Tbl_Brands.Name IS NOT NULL AND Tbl_Brands.Address IS NOT NULL") + ")";
    ...
    public static int Getunread_Messages()
    {
        try
        {
            var db = new DatabaseManagement();
            string unreadMessages =
                string.Format(
                    "SELECT COUNT(MessageID) as Total From Tbl_MailboxFor Where ReceiverID={0} AND ReadStatus IS NULL",
                    IEUtils.ToInt(HttpContext.Current.Session["UserID"]));
            return IEUtils.ToInt(db.GetExecuteScalar(unreadMessages));
        }
        catch (Exception)
