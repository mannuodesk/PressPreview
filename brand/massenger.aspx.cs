...
public partial class brand_massenger : System.Web.UI.Page
{
    ...
    protected string[] GetRecipientID(DatabaseManagement db)
    {
        ...
        if (httpCookie != null)
        {
            ...
            SqlDataReader dr =
            db.ExecuteReader(string.Format("Select SenderID, U_Firstname + ' ' + U_Lastname As [SenderName],U_Email,U_ProfilePic  From Tbl_MailboxMaster INNER JOIN Tbl_Users ON Tbl_Users.UserID=Tbl_MailboxMaster.SenderID  Where ParentID={0}",
    ...
    {
        var userinfo = new string[5];
        SqlDataReader dr =
            db.ExecuteReader(string.Format("Select UserID, U_Firstname + ' ' + U_Lastname as Name,U_Username,U_Email,U_ProfilePic  From Tbl_Users Where U_Username={0}",
        ...
        try
        {
            ...
            if (userCookie != null)
            {
                string query =
                    ...
                    "WHERE MessageID IN (SELECT MAX(MessageID) From Tbl_Mailbox Group By ParentID) AND ReceiverID=" + IEUtils.ToInt(userCookie.Value) + "AND MessageType IS NULL AND  BlockStatus IS NULL  AND Tbl_Mailbox.RDeleted IS NULL ORDER BY DatePosted DESC";
        ...
        try
        {
            ...
            if (userCookie != null)
            {
                string query =
                    ...
                    "FROM Tbl_Mailbox INNER JOIN Tbl_MailboxMaster ON Tbl_MailboxMaster.ParentID=Tbl_Mailbox.ParentID " +
                    "INNER JOIN Tbl_Users ON Tbl_MailboxMaster.SenderID=Tbl_Users.UserID " +
                    "WHERE MessageID IN (SELECT MAX(MessageID) From Tbl_Mailbox Group By ParentID) AND ReceiverID=" + IEUtils.ToInt(userCookie.Value) + "AND MessageType='A' AND  BlockStatus IS NULL  AND Tbl_Mailbox.RDeleted IS NULL ORDER BY DatePosted DESC";
            ...
            if (userCookie != null)
            {
                string query =
                   "SELECT Tbl_Mailbox.ParentID, U_Email,U_ProfilePic,SenderID,ReceiverID,SenderName, Tbl_Users.U_Firstname + ' ' + Tbl_Users.U_Lastname as Name, MessageID,Message,DatePosted,  MessageStatus " +
                    "FROM Tbl_Mailbox INNER JOIN Tbl_MailboxMaster ON Tbl_MailboxMaster.ParentID=Tbl_Mailbox.ParentID " +
                    "INNER JOIN Tbl_Users ON Tbl_MailboxMaster.SenderID=Tbl_Users.UserID " +
                    "WHERE MessageID IN (SELECT MAX(MessageID) From Tbl_Mailbox Group By ParentID) AND ReceiverID=" + IEUtils.ToInt(userCookie.Value) + "AND MessageType='S' AND  BlockStatus IS NOT NULL  AND Tbl_Mailbox.RDeleted IS NULL ORDER BY DatePosted DESC";
    ...
    protected void btnPostMessage_OnServerClick(object sender, EventArgs e)
    {
        try
        {
            ...
            if (userCookie != null)
            {
                // Check if the influencer exists in the brand message list or not
                string checkinfluencer = string.Format(
                    "SELECT UserID  From Tbl_Users Where U_Username={0}  AND U_Type='Editor' AND (UserID IN (SELECT SenderID From Tbl_MailboxMaster Where ReceiverID={1} AND BlockStatus IS NULL) " +
                    "OR  UserID IN (SELECT LikeID From Tbl_BrandsLikes Where UserID={1}) " +
                    "OR  UserID IN (SELECT UserID From  Tbl_Item_Likes Where BrandID={2}) " +
                    "OR  UserID IN (SELECT UserID From Tbl_Posts Where BrandID={3}) " +
                    "OR  UserID IN (SELECT UserID From Tbl_Comments Where BrandID={4})) ",
