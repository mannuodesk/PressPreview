...
public partial class lightbox_item_view : Page
{
    ...
    protected void lbtnWishList_OnClick(object sender, EventArgs e)
    {
        ...
        {
            var db = new DatabaseManagement();
            string addToWishList = string.Format("INSERT INTO Tbl_WishList(ItemID,UserID,DatePosted) VALUES({0},{1},{2})", IEUtils.ToInt(Request.QueryString["v"]), IEUtils.ToInt(Session["UserID"]),"'" + DateTime.UtcNow + "'");
    ...
    protected void WishlistButtonStatus()
    {
        try
        {
            var db = new DatabaseManagement();
            string isAlreadyAdded = "SELECT WishID From Tbl_WishList Where UserID=" + IEUtils.ToInt(Session["UserID"]) + " AND ItemID=" + Convert.ToInt32(Request.QueryString["v"]);
    ...
    protected string GetReceiverBrandName(DatabaseManagement db)
    {
        string getBrandName =
            string.Format(
                "SELECT U_Firstname + ' ' + U_Lastname From Tbl_Users Where UserID=(SELECT UserID From Tbl_Items Where ItemID={0})",
                IEUtils.ToInt(Request.QueryString["v"]));
    ...
    protected void btnGetGift_OnClick(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string[] userInfo = GetRecipientID(db, userNameCookie.Value);
                if (userCookie != null)
                {
                    // Check if the brand already exists in the brand message list or not
                    string checkBrand = string.Format(
                        "SELECT UserID  From Tbl_Users Where U_Firstname + ' ' + U_Lastname={0}  AND U_Type='Brand' AND UserID IN (SELECT ReceiverID From Tbl_MailboxMaster Where SenderID={1} AND BlockStatus IS NULL)",
                        IEUtils.SafeSQLString(GetReceiverBrandName(db)),
                        IEUtils.ToInt(userCookie.Value));
    ...
    protected void btnPostMessage_OnClick(object sender, EventArgs e)
    {
        ErrorMessage.ShowErrorAlert(lblStatus, txtMessage.Value, divAlerts);
    }
}