...
public partial class admin_home_Default : System.Web.UI.Page
{
    ...
    protected void LoadData()
    {
        try
        {
            ...
            string selectQuery = string.Format("SELECT * From Tbl_Seasons Where SeasonID={0}", IEUtils.ToInt(Request.QueryString["v"]));
    ...
    {
        string updateQuery = string.Format("Update Tbl_Seasons Set Season={0} Where SeasonID={1}",
       ...
       Common.EmptyTextBoxes(this);
        LoadData();
        ErrorMessage.ShowSuccessAlert(lblStatus, "Season record updated.", divAlerts);
