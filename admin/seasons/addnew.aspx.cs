...
public partial class admin_home_Default : System.Web.UI.Page
{
    ...
    private void UpdateQuery(DatabaseManagement db)
    {
        ...
        if(db.RecordExist("SELECT SeasonID From Tbl_Seasons Where Season=" + IEUtils.SafeSQLString(txtName.Value.Trim())))
        {
            ErrorMessage.ShowErrorAlert(lblStatus, "Season name already exist", divAlerts);
        }
        else
        {
            updateQuery =
                    string.Format("INSERT INTO Tbl_Seasons(Season) VALUES({0})",
            ...
            ErrorMessage.ShowSuccessAlert(lblStatus, "Season record saved.", divAlerts);            
        }               
