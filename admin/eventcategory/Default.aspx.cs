    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if(e.CommandName=="2")
            {
                ...
                String deleteQuery = string.Format("Delete From Tbl_EventCategory  Where ECategoryID={0}", RecordID);
                db.ExecuteSQL(deleteQuery);
                GridView1.DataBind();
                ErrorMessage.ShowSuccessAlert(lblStatus, "Event Category Deleted", divAlerts);
            }
        }
        catch (Exception ex)
        {
