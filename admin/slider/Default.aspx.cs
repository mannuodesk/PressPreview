    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if(e.CommandName=="2")
            {
                ...
                String deleteQuery = string.Format("Delete From Tbl_ActivityBanners  Where BannerID={0}", RecordID);
                db.ExecuteSQL(deleteQuery);
                GridView1.DataBind();
                ErrorMessage.ShowSuccessAlert(lblStatus, "Slider Image Deleted", divAlerts);
            }
        }
        catch (Exception ex)
        {
