    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if(e.CommandName=="2")
            {
                ...
                String deleteQuery = string.Format("Delete From Tbl_Users  Where UserID={0}", RecordID);
                db.ExecuteSQL(deleteQuery);
                string deleteBrand = string.Format("Delete From Tbl_Editors Where UserID={0}", RecordID);
                db.ExecuteSQL(deleteBrand);
                GridView1.DataBind();
                ErrorMessage.ShowSuccessAlert(lblStatus, "Influencer record deleted", divAlerts);
            }
        }
        catch (Exception ex)
        {
